...
$press_cats = array('press-releases', 'news'); ?>
	...
	<div class="inner-wrapper">
		<h1 class="eight-col"><?php if(is_taxonomy('topic', 'category')) : echo 'We have found '; endif ; ?><?php printf( __( ' %d %s' ), $wp_query->found_posts, _n( 'result', 'results', $wp_query->found_posts), get_posts() ); ?> for "<?php if(!is_tax('group')) : ?><?php $term = get_term_by( 'slug', get_query_var( 'term' ), get_query_var( 'taxonomy' ) ); ?><?php if( $term != '' ) : ?><?php if(!is_tax('topic')) : ?><span class="taxonomy-title"><?php echo $term->name; ?>&nbsp;</span><?php endif; ?><?php endif; ?><?php if( $year != '' ) : ?><?php echo $year, ' '; ?><?php endif; ?><?php endif ; ?><span><?php single_cat_title(); ?></span><?php if(is_taxonomy('topic', 'category')) : echo '"'; endif ; ?></h1>
	</div><!-- /.inner-wrapper -->
</div><!-- /.row -->
